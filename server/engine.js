...
export async function createLesson(library, input, plan) {
  ...
  const budgetPerSource = Math.min(
    15000,
    Math.floor(45000 / Math.max(1, sources.length)),
  );
  const context = {
    profile: (await library.profile()).slice(0, 15000),
    brief: request.brief,
    sources: sources.map((s) => ({
      id: s.id,
      title: s.title,
      excerpt: s.content.slice(0, budgetPerSource),
      truncated: s.content.length > budgetPerSource,
    })),
  };
  const id = randomUUID();
  ...
  const lesson = {
    id,
    title: request.topic,
    summary: request.goal || "A lesson shaped around your question.",
    request,
    context,
    settings,
    ...
    createdAt: now,
    updatedAt: now,
    ...(plan ? lessonPlanSchema.parse(plan) : {}),
  };
  ...
  await sharp(Buffer.from(thumbnailSvg(lesson.title, lesson.style)))
    .png()
    .toFile(path.join(library.lessonDir(id), "thumbnail.png"));
  return lesson;
}
export function startWorker(library, id) {
  ...
  const child = spawn(
    process.execPath,
    [cli, "run-job", id, "--library", library.root],
    {
      detached: true,
      stdio: "ignore",
  ...
  child.on("error", async (e) => {
    ...
    await library.saveLesson({
      ...lesson,
      status: "error",
      stage: "Could not start generation",
      error: e.message,
    });
  });
  child.unref();
...
export async function runJob(
  library,
  id,
  { planGenerator = generatePlan, renderer = renderLesson } = {},
) {
  return library.withLessonLock(id, async () => {
    ...
    try {
      ...
      await writeJson(
        path.join(library.lessonDir(id), "storyboard.json"),
      ...
      await update({ ...result, workerPid: null });
      return lesson;
    } catch (error) {
      await update({
        status: "error",
        stage: "This lesson needs attention",
        error: error.message,
        workerPid: null,
      });
      throw error;
...
export async function askLesson(library, id, question, seconds = 0) {
  if (
    ...
    throw new Error("Ask a question under 3,000 characters.");
  return library.withLessonLock(id, async () => {
    const lesson = await library.lesson(id);
    if (lesson.status !== "ready")
      throw new Error(
        "Wait until this lesson is ready before asking a question.",
      );
    ...
    await writeJson(path.join(library.lessonDir(id), "chat.json"), messages);
    return messages;
  });
}
  ...
  return library.withLessonLock(id, async () => {
    ...
    if (lesson.workerPid) {
      try {
        ...
        throw new Error("This lesson is still being generated.");
      } catch (e) {
