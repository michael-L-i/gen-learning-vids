...
export async function renderScientific(
  ...
  const manifest = scientificManifest.parse(
  ...
  try {
    ...
    await update({
      scenes: timeline,
      stage: "Drawing scientific animation",
      progress: 25,
    });
    progress("Rendering diagrams and graphs with Matplotlib");
    await run(
      python,
      [
        fileURLToPath(new URL("./host.py", import.meta.url)),
        "--module",
        path.join(work, "source", "scene.py"),
        "--timeline",
        path.join(work, "timeline.json"),
        "--output",
        work,
      ],
      { cwd: work, timeout: 30 * 60 * 1000 },
    );
    ...
    await update({
      status: "error",
      workerPid: null,
      stage: "Scientific rendering failed",
      error: error.message,
    });
    throw error;
  }
