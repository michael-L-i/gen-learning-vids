...
const fixture = JSON.parse(
  await fs.readFile(
    new URL(
      "../benchmarks/references/physics-kinematics.json",
      import.meta.url,
    ),
  ),
);
...
test("all reference cases validate and text layout uses actual font measurements", async () => {
  for (const c of await benchmarkCases()) {
    const scene = sceneSchema.parse(
      JSON.parse(
        await fs.readFile(
          new URL(`../benchmarks/references/${c.id}.json`, import.meta.url),
        ),
      ),
    );
    const prepared = await prepareAnimation(scene.content),
      timeline = beatTimeline(scene.content);
    assert.deepEqual(inspectAnimation(prepared, timeline).warnings, [], c.id);
    assert.notEqual(
      animationSvg(prepared, timeline, 0),
      animationSvg(prepared, timeline, 10),
      c.id,
    );
  }
...
test("benchmark agent run renders actual media, preserves timing, replays and stores feedback independently", async (t) => {
  if (!(await available("ffmpeg")) || !(await available("ffprobe")))
    return t.skip("FFmpeg required");
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "benchmark-test-"));
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  const lib = await new Library(root).init();
  const record = await createBenchmarkRun(lib, {
    mode: "agent",
  ...
  const result = await executeBenchmarkRun(lib, record.id, {
    agent: async (_lib, prompt) => {
      calls++;
      assert.match(prompt, /constant acceleration/);
  ...
  assert.equal(item.timeline[1].start, 23 / 30);
  assert.ok(Math.abs(item.duration - 2.3) < 0.001);
  assert.ok(
    item.media.streams.some(
      (s) =>
        s.codec_type === "video" &&
        s.width === 1280 &&
  ...
  const dir = caseDir(lib, record.id, item.id);
  const captions = await fs.readFile(path.join(dir, "captions.vtt"), "utf8");
  assert.match(captions, /00:00:00.767 --> 00:00:01.533/);
  assert.match(
    await fs.readFile(path.join(dir, "transcript.md"), "utf8"),
    /sixteen meters/,
  );
  const feedback = {
    notes: "Arrow tracks cart correctly",
    seconds: 1,
    verdict: "keep",
    scores: { correctness: 4, clarity: 4, layout: 4, motion: 4, style: 4 },
  };
  await saveBenchmarkFeedback(lib, record.id, item.id, feedback);
  assert.equal(
  ...
  assert.deepEqual(
    await readJson(
      path.join(caseDir(lib, replay.id, item.id), "storyboard.json"),
    ),
    sceneSchema.parse(scene),
  );
  assert.equal(replay.cases[0].inputHash, item.inputHash);
  await assert.rejects(
    createBenchmarkRun(lib, { mode: "reference", caseIds: ["../bad"] }),
    /valid benchmark/,
  );
});
...
test("rendered path reveal grows spatially, and code indentation survives text layout", async () => {
  ...
  const code = JSON.parse(
    await fs.readFile(
      new URL("../benchmarks/references/cs-stack.json", import.meta.url),
    ),
  );
